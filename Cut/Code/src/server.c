...
void ToAcceptConnections()
{
	while(1)
	{
	    ...
	    if (pid == 0)
	    {
	    	char filename[200]="" ;
	    	char type[200]="";
	    	ToAuthenticateUser(type);                                //calling the authenticate function to check the username is authentic user
	    	printf("\n%s,after ToAuthenticateUser\n",type);
			char type1[200]="";        
			...
			printf("\n%s,before sending in toaccept connections\n",type1);
...
void AuthenticatedUserFunctionalities(char *recvdata,char *filename ,char * type)
{
	...
	if(strcmp(comm,"ADD")==0)
	{
		...
		{
			char negative[200]="The user can't add the data to the public group";
    ...
    {                                   				                //checking if the command is remove
         printf("\ninside remove\n");
         int res=ToRemoveContact(input1,filename); 
         //strcpy(filename,input1);                             				//calling the user function to remove the contact
         ...
         {
              char neg[200]="Entered user is not present";
              send(connectfd,neg,strlen(neg),0);
         }
    }
    ...
    {                                   				                //checking if the command is remove
         printf("\ninside list\n");
         char concat[1024]="";
         int res=Tolistcontact(input1,filename,concat); 
         //strcpy(filename,input1);                             				//calling the user function to remove the contact
         //if(strcmp(filename,"")!=0)
         if(res == 1)
         {
...
void AdminFunction(char * recvdata,char * filename)
{
      ...
      {
            filename = input3;
            int res = ToAdminAddData(input1,input2,filename);
            if(res==1)
            {
                  char * positive = "";
